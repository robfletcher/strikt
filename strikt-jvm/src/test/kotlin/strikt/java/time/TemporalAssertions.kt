...
internal class TemporalAssertions : JUnit5Minutests {
  companion object {
    ...
    val instant: Instant =
      LocalDateTime.of(2019, 11, 29, 16, 43)
        .atZone(zone)
        .toInstant()
    ...
    rootContext {
      ...
      context("isSameInstant assertion") {
        listOf<Pair<TemporalAccessor, TemporalAccessor>>(
          ...
          test(
            "passes asserting $subject (${subject.javaClass.simpleName}) is the same instant as $expected (${expected.javaClass.simpleName})"
          ) {
        ...
        listOf<Pair<TemporalAccessor, TemporalAccessor>>(
          ...
          test(
            "fails asserting $subject (${subject.javaClass.simpleName}) is the same instant as $expected (${expected.javaClass.simpleName})"
          ) {
            assertThrows<AssertionFailedError> {
              expectThat(subject).isSameInstant(expected)
            }
